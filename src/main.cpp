...
{ // funkce pro celé menu
    ...
    MenuState state = MenuState::MainMenu; // nastaví aktuální stav aplikace (začínáme v hlavním menu)
    ...
    Component component = CatchEvent(renderer, [&](Event event) { // zachytává klávesy / vstupy
        ...
        { // pokud jsme v hlavním menu
            ...
            { // pokud uživatel stiskne ENTER
                if (main_selected == 0)
                { // pokud je vybráno "Start hry"
...
void Combat(ScreenInteractive &screen, player &p)
{
    ...
    Component renderer = Renderer(empty, [&]
                                  { return p.render(); });
